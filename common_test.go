...
func TestDharitriEI_validateToken(t *testing.T) {
	var result bool
	result = ValidateToken([]byte("ANDRIDEFL-08d8eff"))
	assert.False(t, result)
	result = ValidateToken([]byte("ANDRIDEFL-08d8e"))
	assert.False(t, result)
	result = ValidateToken([]byte("ANDRIDEFL08d8ef"))
	assert.False(t, result)
	result = ValidateToken([]byte("ANDRIDEFl-08d8ef"))
	assert.False(t, result)
	result = ValidateToken([]byte("ANDRIDEF*-08d8ef"))
	assert.False(t, result)
	result = ValidateToken([]byte("ANDRIDEFL-08d8eF"))
	assert.False(t, result)
	result = ValidateToken([]byte("ANDRIDEFL-08d*ef"))
	assert.False(t, result)
	...
	result = ValidateToken([]byte("ANDRIDEF2-08d8ef"))
